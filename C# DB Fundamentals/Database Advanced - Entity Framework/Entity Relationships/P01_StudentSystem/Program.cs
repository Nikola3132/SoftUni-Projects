    ...
    class Program
    {
        static void Main(string[] args)
        {
            using (var dbContext = new StudentSystemContext())
            {
                ...
                {
                    Console.WriteLine($"The student's name is {student.Name}. He is registered on {student.RegisteredOn} and his homeworks are: ");

                    foreach (var homework in student.HomeworkSubmissions)
                    {
                        Console.WriteLine($"---- Link- {homework.Content}");
                        Console.WriteLine($"------ {nameof(homework.ContentType)}");
                        Console.WriteLine($"-- His Course is: ");

                        Console.WriteLine($"Course Name - {homework.Course.Name}, Price - {homework.Course.Price}, StartDate - {homework.Course.StartDate}");

                        foreach (var resource in homework.Course.Resources)
                        {
                            Console.WriteLine($"The resource name is {resource.Name} \n The resource type is {resource.ResourceType} \n You can see more info on {resource.Url}");
                        }
                    }
                }
            }
